...
public class EarthquakeCityMap extends PApplet {
	...
	public void draw() {
		background(0);
		map.draw();
		/*The line map.draw() tells the UnfoldingMap to redraw itself, 
		 * making all classes that implement the Marker interface, 
		 * like SimplePointMarker, 
		 * to redraw itself, i.e. call the draw(PGraphics pg, float x, float y) method.*/
		addKey();
	...
	private void addKey() {	
		...
		fill(211, 111, 111);
		triangle(38, 105 -12, 38 -10,  105 + 5, 38 + 10, 105 + 5);
		fill(255, 255, 255);
		ellipse(38, 125, 20, 20);
		rect(28, 140, 20, 20);

		fill(color(255, 255, 0));
		ellipse(38, 200, 20, 20);
		fill(color(0, 0, 255));
		ellipse(38, 225, 20, 20);
		fill(color(255, 0, 0));
		ellipse(38, 250, 20, 20);
		/*fill(color(255, 0, 0));
		        ...
		        for (Marker cm : countryMarkers) {
		        		...
		        		for(Marker m : quakeMarkers) {
		        			if(((EarthquakeMarker)m).isOnLand) {
		        				//if(((LandQuakerMarker)m).)
		        				LandQuakeMarker lm = (LandQuakeMarker) m;
		        				String country = (String) lm.getCountry();
